...
import LikeButton from "../likeButton/likeButton";
 ...
 const Card = ({id,title,image,doctorID,name,content,date,likes,catid,type,isLiked}) => {
...
<a href='/'>{doctorID.user_name}</a>
          </div>  
          <div className='tag'>
            <BsBookmark className='icon' size="2.5vmin" />
            <a href='/'>{name}</a>
          </div>  
          {type==="content"?          <p className="content">{content.slice(0, 100)}...</p>
 :          <p className="content">{content.slice(0, 180)}...</p>

          }
          <div className='date'>
            <AiOutlineCalendar className='icon' /> <label htmlFor=''>{date}</label>
