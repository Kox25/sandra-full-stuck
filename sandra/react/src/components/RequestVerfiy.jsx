...
import doctorss from '../assets/doctoricon.png';
import medalia from '../assets/medaliaicon.png';
import Filee from '../assets/upload-file.png';
import sad from '../assets/sadface.png';
import sandralogo from '../assets/sandralogo.png';
...
export default function RequestVerfiy() {
    ...
    const AcceptOrRejectDoctorRequest = async (doctorId,action) =>
    {
        try
        {
           await axiosClient.post(`/AcceptOrReject/${doctorId}`,{action : action});
        ...
        <div>
            <div className='main mt-[30px] ml-[11%]'>
                ...
                <div className='w-[90%]'>
                    <div className='w-[20%]'>
                        <div className='flex mt-9 '>
                            ...
                            <p className='mt-[60px] mt-8 ml-7 flex adminsection'>{localStorage.getItem('user-name')}</p>
                        </div>
                    </div>
                    ...
                    <div className='w-[80%] mt-[-25px]'>
                        {doctorsInfo.length === 0 ? (
                            <div className='ml-[60%] mt-[8%]'>
                                <img className='mb-2 ml-[19%]' src={sad} alt='sad-icon' style={{ width: '70px', height: '70px' }} />
                                <p className='messagee mr-[40%] text-lg'><span className='text-xl'>Opps..</span>No requests</p>
                            </div>
                        ) : (
                            <ul className='h-[370px] w-[130%]' style={{ overflowX: 'hidden' }}>
                                {doctorsInfo.map((doctor) => (
                                    <li key={doctor.id} className='itemm mb-12 h-[150px] flex items-center ml-[230px]'>
                                        <div>
                                            <img src={doctorss} className='doctorRequest' alt='Doctor Icon' />
                                        </div>
                                        <img src={medalia} className='medaliaicon ml-[-33px] mb-[90px]' alt='Medalia Icon' />
                                        <div className='ml-4 verfiyinfo'>
                                            <p align='left' className='mb-2'>Name: dr.{doctor.user_name}</p>
                                            <p align='left' className='mb-2'>Request: verify</p>
                                        </div>
                                        ...
                                        <div className='ml-[70px] flex buttons'>
                                            <button className='mr-12 acceptbutton w-[80px]' onClick={()=> {AcceptOrRejectDoctorRequest(doctor.id ,"accept")}}>Accept</button>
                                            <button className='rejectbutton w-[80px]' onClick={()=> {AcceptOrRejectDoctorRequest(doctor.id , "reject")}}>Reject</button>
                                        </div>
                                    </li>
                                ))}
                            </ul>
                ...
                <div className='flex mr-[900px] signture'>
                    <p className='mt-2'>Admin {localStorage.getItem('user-name')}</p>
